...
- [ ] ESP32의 `SERVER_URL`/`HEARTBEAT_URL`을 로컬 IP → Vercel 배포 주소로 교체 후 전송 테스트 (Vercel 배포 후 진행 예정)
...
- [ ] 반응형 디자인 최종 점검 (모바일 최적화, 육안 확인은 됐으나 다양한 화면 크기 테스트는 미실시)

### Phase 6. PWA 전환 및 배포 (1일)
- [ ] `next-pwa` 적용, manifest.json / 아이콘 설정
- [ ] Vercel에 배포
...
- ~~이용 이력 기반 "시간대별 혼잡도" 통계 대시보드~~ → **이력 데이터 적재(`room_status_log`)는 구현 완료**, 통계를 보여주는 화면(대시보드)은 아직 미구현 — Phase 5 이후 여유 있으면 추가
- 다른 층/다른 회의실로 확장 적용
- 슬랙(Slack) 연동 알림 (상태 변경 시 채널에 자동 메시지)
- 재실 인원 수 감지 (초음파/카메라 센서 추가 시)

---

*이 계획서는 진행 상황에 따라 계속 업데이트하며 사용하시면 됩니다.*
